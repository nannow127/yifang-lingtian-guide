...
const rows = (name) => XLSX.utils.sheet_to_json(workbook.Sheets[name], { header: 1, defval: '' });
...
const npcNames = ['李梦卿', '陈远舟', '卫泓', '杨子勤', '牧夏', '宋烟瞳', '纪瑶华', '游景和', '谢闻天', '左离', '林婉兮', '叶玉心'];
...
const recipes = recipeRows
  ...
  .map((row) => {
    ...
    const staminaValues = [...effect.matchAll(/(\d+)\s*点体力/g)].map((match) => Number(match[1]));
    const slashValues = [...effect.matchAll(/(\d+)\s*\/\s*(\d+)/g)].flatMap((match) => [Number(match[1]), Number(match[2])]);
    return {
      id: number(row[0]),
      name: clean(row[2]),
      effect,
      ...
      preferences: Object.fromEntries(npcNames.map((npc, index) => [npc, clean(row[index + 5]) || '未知'])),
      unlock: clean(row[18]),
    };
  });
...
const fish = rows('【钓鱼图鉴】')
  ...
  .map((row) => ({
    ...
    locations: row
      .slice(5, 14)
      .map((value, index) => (clean(value) === '√' ? clean(fishHeader[index + 5]).replace(/\n/g, '') : ''))
      .filter(Boolean),
  }));
...
const pillCategory = (effect) => {
  ...
  if (/提升\d+点|提升\d+.*上限/.test(effect) && !/小时/.test(effect)) return '永久提升';
  if (/钓鱼|工具|采集|加工|烹饪|收获|秘境的入口/.test(effect)) return '生活辅助';
  if (/法术|普通攻击|灵力消耗|道法值|身法值|神识值|定力值/.test(effect)) return '战斗增益';
...
const sourceMap = new Map(materials.map((item) => [item.name, { source: item.source, type: item.type }]));
crops.forEach((crop) =>
...
const usedIngredients = [...new Set(recipes.flatMap((recipe) => recipe.ingredients.map((item) => item.name)))].sort((a, b) => a.localeCompare(b, 'zh-CN'));
